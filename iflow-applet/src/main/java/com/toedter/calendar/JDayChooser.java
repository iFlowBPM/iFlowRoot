...
public class JDayChooser extends JPanel implements ActionListener, KeyListener,
		...
		for (int y = 0; y < 7; y++) {
			for (int x = 0; x < 7; x++) {
				...
				if (y == 0) {
					...
					days[index] = new JButton("x") { //$NON-NLS-1$
						private static final long serialVersionUID = -7433645992591669725L;

						public void paint(Graphics g) {
							if ("Windows".equals(UIManager.getLookAndFeel() //$NON-NLS-1$
									.getID())) {
	...
	protected void drawWeeks() {
		...
		for (int i = 1; i < 7; i++) {
			...
			if (week < 10) {
				buttonText = "0" + buttonText; //$NON-NLS-1$
			}
	...
	public void setDay(int d) {
		...
		if (alwaysFireDayProperty) {
			firePropertyChange("day", 0, day); //$NON-NLS-1$
		} else {
			firePropertyChange("day", oldDay, day); //$NON-NLS-1$
		}
	}
	...
	public void updateUI() {
		super.updateUI();
		setFont(Font.decode("Dialog Plain 11")); //$NON-NLS-1$
	...
	public static void main(String[] s) {
		JFrame frame = new JFrame("JDayChooser"); //$NON-NLS-1$
