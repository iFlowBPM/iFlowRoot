...
public class JSpinnerDateEditor extends JSpinner implements IDateEditor,
	...
	public void setDate(Date date, boolean updateModel) {
		...
		this.date = date;
		if (date == null) {
			((JSpinner.DateEditor) getEditor()).getFormat().applyPattern(""); //$NON-NLS-1$
			((JSpinner.DateEditor) getEditor()).getTextField().setText(""); //$NON-NLS-1$
		} else if (updateModel) {
	...
	public void setEnabled(boolean b) {
		...
		if (!b) {
			((JSpinner.DateEditor) getEditor()).getTextField().setBackground(
					UIManager.getColor("TextField.inactiveBackground")); //$NON-NLS-1$
		}
	}
