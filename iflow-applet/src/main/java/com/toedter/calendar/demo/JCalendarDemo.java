...
public class JCalendarDemo extends JApplet implements PropertyChangeListener {
	...
	public void init() {
		...
		URL iconURL = beans[0].getClass().getResource(
				"images/" + beans[0].getName() + "Color16.gif"); //$NON-NLS-1$ //$NON-NLS-2$
		ImageIcon icon = new ImageIcon(iconURL);

		propertyTitlePanel = new JTitlePanel("Properties", null, propertyPanel, BorderFactory //$NON-NLS-1$
				.createEmptyBorder(4, 4, 4, 4));

		componentTitlePanel = new JTitlePanel("Component", icon, componentPanel, BorderFactory //$NON-NLS-1$
	...
	public final void initializeLookAndFeels() {
		...
		try {
			...
			for (int i = 0; i < lnfs.length; i++) {
				if (lnfs[i].getName().equals("JGoodies Plastic 3D")) { //$NON-NLS-1$
					found = true;
				}
			}
			if (!found) {
				UIManager.installLookAndFeel("JGoodies Plastic 3D", //$NON-NLS-1$
						"com.jgoodies.looks.plastic.Plastic3DLookAndFeel"); //$NON-NLS-1$
			}
	...
	public JToolBar createToolBar() {
		...
		toolBar.putClientProperty("jgoodies.headerStyle", "Both"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		for (int i = 0; i < beans.length; i++) {
			...
			try {
				...
				URL iconURL = bean.getClass().getResource(
						"images/" + bean.getName() + "Color16.gif"); //$NON-NLS-1$ //$NON-NLS-2$
				...
				System.out.println("JCalendarDemo.createToolBar(): " + e); //$NON-NLS-1$
	...
	public JMenuBar createMenuBar() {
		...
		JMenu componentsMenu = new JMenu("Components"); //$NON-NLS-1$
		...
		for (int i = 0; i < beans.length; i++) {
			...
			try {
				URL iconURL = beans[i].getClass().getResource(
						"images/" + beans[i].getName() + "Color16.gif"); //$NON-NLS-1$ //$NON-NLS-2$
				icon = new ImageIcon(iconURL);
				menuItem = new JMenuItem(beans[i].getName(), icon);
			} catch (Exception e) {
				System.out.println("JCalendarDemo.createMenuBar(): " + e + " for URL: " + "images/" //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
						+ beans[i].getName() + "Color16.gif"); //$NON-NLS-1$
		...
		JMenu lnfMenu = new JMenu("Look&Feel"); //$NON-NLS-1$
		lnfMenu.setMnemonic('L');

		menuBar.add(lnfMenu);

		for (int i = 0; i < lnfs.length; i++) {
			if (!lnfs[i].getName().equals("CDE/Motif")) { //$NON-NLS-1$
						...
						new ItemListener() {
							public void itemStateChanged(ItemEvent ie) {
								...
								if (rbmi2.isSelected()) {
									...
									try {
										menuBar.putClientProperty("jgoodies.headerStyle", "Both"); //$NON-NLS-1$ //$NON-NLS-2$
										...
										System.err.println("Unable to set UI " + e.getMessage()); //$NON-NLS-1$
									}
								}
							}
						});
	...
	public void propertyChange(PropertyChangeEvent evt) {
		if (calendarPanel != null) {
			if (evt.getPropertyName().equals("calendar")) { //$NON-NLS-1$
	...
	public static void main(String[] s) {
		...
		JFrame frame = new JFrame("JCalendar Demo"); //$NON-NLS-1$
	...
	private void installBean(JComponent bean) {
		try {
			...
			String[] types = new String[] { "class java.util.Locale", "boolean", "int", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
					"class java.awt.Color", "class java.util.Date", "class java.lang.String" }; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
				...
				for (int i = 0; i < propertyDescriptors.length; i++) {
					if (propertyDescriptors[i].getWriteMethod() != null) {
						...
						if (type.equals(types[t])
								&& (((readMethod != null) && (writeMethod != null)) || ("class java.util.Locale" //$NON-NLS-1$
										.equals(type)))) {
							if ("boolean".equals(type)) { //$NON-NLS-1$
								...
								final JCheckBox checkBox = new JCheckBox("", isSelected); //$NON-NLS-1$
								...
								spinField.addPropertyChangeListener(new PropertyChangeListener() {
									public void propertyChange(PropertyChangeEvent evt) {
										try {
											if (evt.getPropertyName().equals("value")) { //$NON-NLS-1$
								...
								count += 1;
							} else if ("class java.lang.String".equals(type)) { //$NON-NLS-1$
								...
								count += 1;
							} else if ("class java.util.Locale".equals(type)) { //$NON-NLS-1$
								...
								count += 1;
							} else if ("class java.util.Date".equals(type)) { //$NON-NLS-1$
								Date date = null;
								...
								count += 1;
							} else if ("class java.awt.Color".equals(type)) { //$NON-NLS-1$
									...
									ActionListener actionListener = new ActionListener() {
										public void actionPerformed(ActionEvent e) {
											Color newColor = JColorChooser.showDialog(
													JCalendarDemo.this, "Choose Color", colorObj); //$NON-NLS-1$
			...
			URL iconURL = bean.getClass().getResource("images/" + bean.getName() + "Color16.gif"); //$NON-NLS-1$ //$NON-NLS-2$
			ImageIcon icon = new ImageIcon(iconURL);
	...
	private void addProperty(PropertyDescriptor propertyDescriptor, JComponent editor,
		...
		JLabel label = new JLabel(newText + ": ", null, JLabel.RIGHT); //$NON-NLS-1$
		GridBagConstraints c = new GridBagConstraints();
		c.weightx = 1.0;
		c.fill = GridBagConstraints.BOTH;
