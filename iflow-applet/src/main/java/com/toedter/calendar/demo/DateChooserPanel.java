...
public class DateChooserPanel extends JPanel implements PropertyChangeListener {
	...
	public DateChooserPanel() {
		setName("JDateChooser"); //$NON-NLS-1$
		...
		components[3] = new JDateChooser("dd/MM/yyyy", "##/##/####", ' '); //$NON-NLS-1$ //$NON-NLS-2$
		((JDateChooser)components[3]).setDate(new Date());
		components[4] = new DemoTable();

		addEntry("Default", components[0], gridbag); //$NON-NLS-1$
		addEntry("Default with date set", components[1], gridbag); //$NON-NLS-1$
		addEntry("Spinner Editor", components[2], gridbag); //$NON-NLS-1$
		addEntry("Explicite date pattern and mask", components[3], gridbag); //$NON-NLS-1$
		addEntry("Table with date editors", components[4], gridbag); //$NON-NLS-1$
	}

	private void addEntry(String text, JComponent component, GridBagLayout grid) {
		JLabel label = new JLabel(text + ": ", null, JLabel.RIGHT); //$NON-NLS-1$
		GridBagConstraints c = new GridBagConstraints();
		c.weightx = 1.0;
		c.fill = GridBagConstraints.BOTH;
