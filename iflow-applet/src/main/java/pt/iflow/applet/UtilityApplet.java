 ...
 * carregamento, actualização e verificação de ficheiros. A api é
 * suficientemente genérica para suportar operações de criptografia como
 * assinaturas digitais e (des)encriptação de ficheiros. Também suporta
 * desencriptação de dados de processo que tenham sido previamente encriptados.
 * 
 * @author Óscar Lopes
 *
 */
public class UtilityApplet extends JApplet implements UtilityConstants {
	...
	/**
	 * Inicialização da applet. Colocar aqui código de activação de scanner,
	 * etc.
	 */
	public void init() {
		super.init();
	...
	/**
	 * Arranque da applet. Método chamado quando a applet termina o processo de
	 * inicialização e está pronta a ser apresentada ao utilizador.
	 */
	public void start() {
		super.start();
		...
		System.out.println("  " + System.getProperty("os.name") + " (" + System.getProperty("os.arch") + ") " //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
				+ System.getProperty("os.version")); //$NON-NLS-1$

	}

	/**
	 * Término da applet. Chamado quando a página do browser muda ou é fechada.
	 * Usado para libertar recursos.
	 */
	public void stop() {
		super.stop();
	 ...
	 * 
	 * @param fileFormat
	 *            file format to test or <code>null</code> to test any
	 * @return <code>true</code> if can scan, <code>false</code> otherwise.
	 */
	/*
	 * public boolean canScan(final String fileFormat) { // log.info(
	 * "canScan started"); //$NON-NLS-1$ boolean result = false; try { result =
	 * AccessController.doPrivileged(new PrivilegedExceptionAction<Boolean>() {
	 * public Boolean run() { return doCanScan(fileFormat); } }); } catch
	 * (PrivilegedActionException e) { log.error(
	 * "Privilegios insuficientes para executar a operacao pretendida", e);
	 * //$NON-NLS-1$ } log.info("canScan complete"); //$NON-NLS-1$ return
	 * result; }
	 */
	 ...
	 */
	/*
	 * public boolean canSign(final String signatureType) { // log.info(
	 * "canSign started"); //$NON-NLS-1$ boolean result = false; try { result =
	 * AccessController.doPrivileged(new PrivilegedExceptionAction<Boolean>() {
	 * public Boolean run() { return doCanSign(signatureType); } }); } catch
	 * (PrivilegedActionException e) { log.error(
	 * "Privilegios insuficientes para executar a operacao pretendida", e);
	 * //$NON-NLS-1$ } log.info("canSign complete"); //$NON-NLS-1$ return
	 * result; }
	 */
	 ...
	 */
	/*
	 * public boolean canEncrypt(final String encryptType) { // log.info(
	 * "canEncrypt started"); //$NON-NLS-1$ boolean result = false; try { result
	 * = AccessController.doPrivileged(new PrivilegedExceptionAction<Boolean>()
	 * { public Boolean run() { return doCanEncrypt(encryptType); } }); } catch
	 * (PrivilegedActionException e) { log.error(
	 * "Privilegios insuficientes para executar a operacao pretendida", e);
	 * //$NON-NLS-1$ } log.info("canEncrypt complete"); //$NON-NLS-1$ return
	 * result; }
	 */
	 ...
	 * @param varName
	 *            Variável onde será guardado o ficheiro
	 * @param signatureType
	 *            Tipo de assinatura a efectuar (NONE, XADES, PDF, PKCS7)
	 * @param encrptionType
	 *            Tipo de encriptação a efectuar (NONE, PKI)
	 * 
	...
	public final String uploadFile(final String cookie, final String jsonRequest, final String sig_pos_style_java) {
		...
		String result = "";
		/*
		 * log.info("Signature_Position_style from javascript, value: "
		 * +sig_pos_style_java);
		 * LoadImageAction.setSignature_position_style(sig_pos_style_java);
		 * 
		 * try { result = AccessController.doPrivileged(new
		 * PrivilegedExceptionAction<String>() { public String run() { try {
		 * return doUploadFile(createWebClient(cookie, jsonRequest)); } catch
		 * (Throwable t) { log.error("Error occurred.", t); //$NON-NLS-1$ }
		 * return null; } }); } catch (PrivilegedActionException e) { log.error(
		 * "Privilegios insuficientes para executar a operacao pretendida", e);
		 * //$NON-NLS-1$ } log.info("uploadFile complete"); //$NON-NLS-1$
		 */
		return result;
	}

	public final String uploadFileFromDisk(final String cookie, final String jsonRequest, final byte[] byteArr,
			final String filename, final String sig_pos_style_java) {
		String result = "";
		// LoadImageAction.setSignature_position_style(sig_pos_style_java);
		/*
		 * try { result = AccessController.doPrivileged(new
		 * PrivilegedExceptionAction<String>() { public String run() { try {
		 * return doUploadFileFromDisk(createWebClient(cookie, jsonRequest),
		 * filename, byteArr); } catch (Throwable t) { log.error(
		 * "Error occurred.", t); //$NON-NLS-1$ } return null; } }); } catch
		 * (PrivilegedActionException e) { log.error(
		 * "Privilegios insuficientes para executar a operacao pretendida", e);
		 * //$NON-NLS-1$ }
		 */
	...
	public final String previewFile(final String cookie, final String jsonRequest) {
		 ...
		 * try { result = AccessController.doPrivileged(new
		 * PrivilegedExceptionAction<String>() { public String run() { try {
		 * return doPreviewFile(createWebClient(cookie, jsonRequest)); } catch
		 * (Throwable t) { log.error("Error occurred.", t); //$NON-NLS-1$ }
		 * return null; } }); } catch (PrivilegedActionException e) { log.error(
		 * "Privilegios insuficientes para executar a operacao pretendida", e);
		 * //$NON-NLS-1$ } log.info("previewFile complete"); //$NON-NLS-1$
		 */
		return result;
	}

	/**
	 * Modificar ficheiro existente. Permite reassinar ou encriptar um documento
	...
	public final String modifyFile(final String cookie, final String jsonRequest, final String sig_pos_style_java) {
		 ...
		 * 
		 * try { result = AccessController.doPrivileged(new
		 * PrivilegedExceptionAction<String>() { public String run() { return
		 * doModifyFile(createWebClient(cookie, jsonRequest)); } }); } catch
		 * (PrivilegedActionException e) { log.error(
		 * "Privilegios insuficientes para executar a operacao pretendida", e);
		 * //$NON-NLS-1$ } log.info("modifyFile complete"); //$NON-NLS-1$
		 */
		return result;
	}
	...
	public final String replaceFile(final String cookie, final String jsonRequest, final String sig_pos_style_java) {
		 ...
		 * try { result = AccessController.doPrivileged(new
		 * PrivilegedExceptionAction<String>() { public String run() { return
		 * doReplaceFile(createWebClient(cookie, jsonRequest)); } }); } catch
		 * (PrivilegedActionException e) { log.error(
		 * "Privilegios insuficientes para executar a operacao pretendida", e);
		 * //$NON-NLS-1$ } log.info("replaceFile complete"); //$NON-NLS-1$
		 */
		return result;
	}

	/**
	 * Descarregar documento existente. Desencriptar um documento previamente
	 * encriptado ou simplesmente efectuar o download do mesmo.
	 * 
	 * @param cookie
	 *            page cookie
	 * @param flowid
	...
	public final String downloadFile(final String cookie, final String jsonRequest) {
		 ...
		 * try { result = AccessController.doPrivileged(new
		 * PrivilegedExceptionAction<String>() { public String run() { return
		 * doDownloadFile(createWebClient(cookie, jsonRequest)); } }); } catch
		 * (PrivilegedActionException e) { log.error(
		 * "Privilegios insuficientes para executar a operacao pretendida", e);
		 * //$NON-NLS-1$ } log.info("downloadFile complete"); //$NON-NLS-1$
		 */
		return result;
	}
	...
	public final String verifiyFile(final String cookie, final String jsonRequest) {
		 ...
		 * try { result = AccessController.doPrivileged(new
		 * PrivilegedExceptionAction<String>() { public String run() { return
		 * doVerifiyFile(createWebClient(cookie, jsonRequest)); } }); } catch
		 * (PrivilegedActionException e) { log.error(
		 * "Privilegios insuficientes para executar a operacao pretendida", e);
		 * //$NON-NLS-1$ } log.info("verifiyFile complete"); //$NON-NLS-1$
		 */
		return result;
	}
	...
	public String removeFile(final String cookie, final String jsonRequest) {
		 ...
		 * try { result = AccessController.doPrivileged(new
		 * PrivilegedExceptionAction<String>() { public String run() { return
		 * doRemoveFile(createWebClient(cookie, jsonRequest)); } }); } catch
		 * (PrivilegedActionException e) { log.error(
		 * "Privilegios insuficientes para executar a operacao pretendida", e);
		 * //$NON-NLS-1$ } log.info("removeFile complete"); //$NON-NLS-1$
		 */
		return result;
	}
	 ...
	 *            Tipo de encriptação a efectuar (NONE, PKI)
	 * @param fileFormat
	 *            Formato do ficheiro (PDF, JPG, TIFF, etc)
	 * 
	 * @return Identificador do ficheiro carregado
	 */
	public final String scanFile(final String cookie, final String jsonRequest) {
		 ...
		 * try { result = AccessController.doPrivileged(new
		 * PrivilegedExceptionAction<String>() { public String run() { return
		 * doScanFile(createWebClient(cookie, jsonRequest)); } }); } catch
		 * (PrivilegedActionException e) { log.error(
		 * "Privilegios insuficientes para executar a operacao pretendida", e);
		 * //$NON-NLS-1$ } log.info("scanFile complete"); //$NON-NLS-1$
		 */
		return result;
	}
	 ...
	 *            Tipo de certificado (autenticação, encriptação, assinatura)
	 * 
	 * @return
	 */
	/*
	 * public JSONObject setCertificate(final String cookie, final String
	 * jsonRequest) { //log.debug("setCertificate called: "+jsonRequest);
	 * //$NON-NLS-1$ JSONObject result = ; try { result =
	 * AccessController.doPrivileged(new PrivilegedExceptionAction<JSONObject>()
	 * { public JSONObject run() { return
	 * doSetCertificate(createWebClient(cookie, jsonRequest)); } }); } catch
	 * (PrivilegedActionException e) { log.error(
	 * "Privilegios insuficientes para executar a operacao pretendida", e);
	 * //$NON-NLS-1$ } log.info("setCertificate complete"); //$NON-NLS-1$ return
	 * result; }
	 */
	...
	private String doGetTaskStatus(String taskId) {
		if (StringUtils.isBlank(taskId))
			return TaskStatus.getInvalidStatus();
	...
	private String doUploadFile(final WebClient webClient) {
		...
		DynamicDialog dialog = new DynamicDialog(this, webClient, signer, cipher,
				new FileDialogProvider(webClient.getVariable(), false));
	...
	private String doReplaceFile(final WebClient webClient) {
		...
		FileCipher cipher = CipherType.getFileCipher(webClient);
		DynamicDialog dialog = new DynamicDialog(this, webClient, signer, cipher,
				new FileDialogProvider(webClient.getVariable(), true));
		TaskStatus task = dialog.openDialog();
		this.tasks.put(task.getTaskId(), task);
		return task.getTaskId();
	}
	...
	private String doScanFile(final WebClient webClient) {
		...
		IVFile theFile = SwingUtils.showOpenFileDialog(this, Messages.getString("UtilityApplet.0"), //$NON-NLS-1$
