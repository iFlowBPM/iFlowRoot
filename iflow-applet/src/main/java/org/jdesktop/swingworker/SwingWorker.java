...
public abstract class SwingWorker<T, V> implements Future<T>, Runnable {
    ...
    protected final void setProgress(int progress) {
        if (progress < 0 || progress > 100) {
            throw new IllegalArgumentException("the value should be from 0 to 100"); //$NON-NLS-1$
        }
        ...
        if (! getPropertyChangeSupport().hasListeners("progress")) { //$NON-NLS-1$
            return;
        }
        synchronized (this) {
                ...
                doNotifyProgressChange = 
                    new AccumulativeRunnable<Integer>() {
                        ...
                        public void run(List<Integer> args) {
                            firePropertyChange("progress",  //$NON-NLS-1$
                               args.get(0), 
    ...
    private void setState(StateValue state) {
        ...
        this.state = state;
        firePropertyChange("state", old, state); //$NON-NLS-1$
    }
            ...
            ThreadFactory threadFactory = 
                new ThreadFactory() {
                    ...
                    public Thread newThread(final Runnable r) {
                        StringBuilder name = 
                            new StringBuilder("SwingWorker-pool-"); //$NON-NLS-1$
                        name.append(System.identityHashCode(this));
                        name.append("-thread-"); //$NON-NLS-1$
