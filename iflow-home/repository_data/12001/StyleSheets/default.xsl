...
li.header {
	background-color: #D94095;
	border-bottom:1px solid #D00773;
	border-right:1px solid #D00773;
	height:19px;
	padding:5px 5px 2px 0;
}
...
<tbody  class="scrollContent">
  ...
  <xsl:template match="a">
	...
	<xsl:if test="not(substring(href/text(),1,15) = '/iFlow/document')">
          <xsl:attribute name="onclick">
            <xsl:text>javascript:camouflageForm();</xsl:text>
          </xsl:attribute>
        </xsl:if>
