  ...
  <xsl:template match="/">
    <html>
      <head>
        ...
        <link rel="stylesheet" type="text/css" href="{$url_prefix}/javascript/lightbox/css/lightbox.css" />
...
* html li.password { height: 20px; }
      ...
      <body onload="initProcFrame();">
		...
		<div id="_formLoadingDiv" style="text-align:center;width:99%;height:99%;position:absolute;left:0;top:0;z-index:99;display:none">
		 ...
		 <script type="text/javascript" src="{$url_prefix}/javascript/lightbox/js/lightbox.js"> </script>
		  <script>
	...
	<xsl:if test="type='selection'">
   	...
   	<xsl:if test="type='radiobutton'">
		...
		<div class="txt">
			...
			<xsl:for-each select="option">
				<input type="radio">
					<xsl:if test="../output_only/text() = 'true'">
      ...
      <xsl:if test="type='singlecheckbox'">
  ...
  <xsl:template match="thumbnail">
   <img src="{src}" data-jslghtbx="" data-jslghtbx-group="group">      
     <xsl:attribute name="alt">
  ...
  <xsl:template match="a">
    <a>
      ...
      <xsl:if test="href/text()='form.jsp'">
      	<xsl:attribute name="onclick">
      		disableForm()
      	</xsl:attribute>  
      </xsl:if>      
