      ...
      for (int i = 0; i < xmlLibrary.getXmlBlock().size(); i++) {
        ...
        if (cbibs.hasLibrary(bib.getName()) || (bib.getComponent(descricao) == null && cbibs.getComponent(descricao) == null)) {
          ...
          for (int j = 0; j < block.getXmlPort().size(); j++) {
            ...
            if (port.getInOut() == InOutType.IN) {
              if (port.getPosition() != null)
                p_e[_i] = new Point(port.getPosition().getX().intValue(), port.getPosition().getY().intValue());
              else
                p_e[_i] = new Point(-10, 10 + 10 * _i);
              _i++;
            } else {
              if (port.getPosition() != null)
                p_s[_j] = new Point(port.getPosition().getX().intValue(), port.getPosition().getY().intValue());
              else
          ...
          for (int j = 0; j < block.getXmlAttribute().size(); j++) {
            ...
            String[] attrString = attr.getValueType().toArray( new String[attr.getValueType().size()] );
            attrs.add(new AtributoImpl(attr.getName(), attr.getValue().getValue(), attr.getDescription(), attrString));
          }
          ...
          if (pos != null) {
            largX = pos.getX().intValue();
            largY = pos.getY().intValue();
          } else {
            int _ins = -15;
