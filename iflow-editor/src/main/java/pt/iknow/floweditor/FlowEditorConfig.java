...
public class FlowEditorConfig {
	...
	@XmlElement( name="list-flow-uRL" )
	protected Vector<FlowRepUrl> listFlowURL = null;
	
	@XmlElement( name="default-url-index" )
	protected Integer defaultUrlIndex = new Integer(-1);

	@XmlAttribute( name="use-proxy" )
	protected boolean useProxy = false;
	@XmlAttribute( name="proxy-host" )
	protected String proxyHost = null;
	@XmlAttribute( name="proxy-port" )
	protected String proxyPort = null;
	@XmlAttribute( name="use-proxy-auth" )
	protected boolean useProxyAuth = false;
	@XmlAttribute( name="proxy-user" )
	protected String proxyUser = null;
	@XmlAttribute( name="proxy-pass" )
	protected String proxyPass = null;
	@XmlAttribute( name="use-nTAuth" )
	protected boolean useNTAuth = false;
	@XmlAttribute( name="proxy-domain" )
	protected String proxyDomain = null;
	@XmlAttribute( name="confirm-exit" )
	protected boolean confirmExit = true;

	// Lang stuff
	@XmlElement( name="use-locale" )
	...
	public void saveConfig() {
		...
		try {
			...
			JAXBContext context = JAXBContext.newInstance(FlowEditorConfig.class);
	...
	public static FlowEditorConfig loadConfig() {
		...
		try {
			...
			feConfig = (FlowEditorConfig) unmarshaller.unmarshal(isr);
			fis.close();
		} catch (Exception e) {
			FlowEditor.log("error", e);
		}
		...
		if (StringUtilities.isEmpty(feConfig.useLocale))
			feConfig.useLocale = LOCALE_USE_IFLOW;
	...
	public static XmlLetterType getLetters() {
		...
		try {
			...
			JAXBContext context = JAXBContext.newInstance(XmlLetterType.class);
			Unmarshaller unmarshaller = context.createUnmarshaller();
			letters = (XmlLetterType) unmarshaller.unmarshal(iSrc);
			is.close();
			is = null;
		} catch (Exception e) {
			FlowEditor.log("error", e);
	...
	public static XmlColorLibrary getColors() {
		...
		try {
			...
			is = new FileInputStream(theFile);
			InputSource iSrc = new InputSource(is);
			JAXBContext context = JAXBContext.newInstance(XmlColorLibrary.class);
			Unmarshaller unmarshaller = context.createUnmarshaller();
			colors = (XmlColorLibrary) unmarshaller.unmarshal(iSrc);
			is.close();
			is = null;
		} catch (Exception e) {
			FlowEditor.log("error", e);
