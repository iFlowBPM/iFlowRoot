...
public class AlteraAtributosWebForm extends AbstractAlteraAtributos implements AlteraAtributosInterface, Runnable {
  ...
  public void setDataIn(String title, List<Atributo> atributos) {
    ...
    addWindowListener(new WindowListener() {
      ...
      public void windowClosing(WindowEvent e) {
        if (JOptionPane.showConfirmDialog(AlteraAtributosWebForm.this, "Quer terminar o bloco?", "Terminar", JOptionPane.YES_NO_OPTION) == JOptionPane.NO_OPTION)
          return;
  ...
  private void createSShell() {
    sShell = new Shell((Shell)adapter.getRootShell(), SWT.APPLICATION_MODAL | SWT.CLOSE | SWT.TITLE | SWT.MAX | SWT.RESIZE);
  ...
  public void run() {
    ...
    browser.addCloseWindowListener(new CloseWindowListener() {
      public void close(org.eclipse.swt.browser.WindowEvent event) {
        adapter.log("Close requested");
        closeWindow(exitStatus);
      }
    });
     */
    browser.setUrl(sURL);
  ...
  public void executeGetCatalog(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    for(int i = 0; i < styleSheets.length; i++) {
      ...
      sb.append("DragNSort.FormStyleSheets.push({description:'").append(str).append("',value:'").append(str).append("'});");
    ...
    for(int i = 0; i < styleSheets.length; i++) {
      StringBuilder sb = new StringBuilder();
      String str = StringEscapeUtils.escapeJavaScript(styleSheets[i]);
      sb.append("DragNSort.PrintStyleSheets.push({description:'").append(str).append("',value:'").append(str).append("'});");
      adapter.log(sb.toString());
      pw.println(sb);
    }
  ...
  public void executeLoadMessages(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    while(keys.hasMoreElements()) {
      ...
      pw.println("FormEditorMessages.messages['"+StringEscapeUtils.escapeJavaScript(key.substring(len))+"']='"+StringEscapeUtils.escapeJavaScript(msg)+"';");
    }
  ...
  private String generateHtmlPost(String url, String form, String catalog) {
    if(null == form || null == catalog) return null;
  ...
  public void executeDoPromote(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    try {
      ...
      if(desenho.getFormTemplate(name) != null) exists = true;
      if(force) exists = false;
