...
public class BlockVFSGenerateSerialCode extends Block {
  public Port portIn, portTrue, portFalse;

  private final String sATTR_SLIST = "template";
  private final String sATTR_TLIST = "serial";
  
  public BlockVFSGenerateSerialCode(int anFlowId,int id, int subflowblockid, String filename) {
      ...
      if (StringUtils.isEmpty(sTemplate) || StringUtils.isEmpty(sSerial) && null == sSerial && null == sTemplate) {
        Logger.warning(login, this, "after", procData.getSignature() + "empty attribute!!");
    	 ...
    	 try {
          ...
          if( template != null )
          {
        	  String temp = template.getRawValue();
              CodeTemplateManager ctm = BeanFactory.getCodeTemplateManagerBean();
        	  if(ctm != null)
        	  {
        		  String ctmString = ctm.generateSerialCode(userInfo, temp );
        		  
        		  if( serial != null )
        		  {
        			  serial.setValue( ctmString );
        		  }
        	  }        			  
          }
