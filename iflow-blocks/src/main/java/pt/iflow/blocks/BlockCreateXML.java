...
public class BlockCreateXML extends Block{
	...
	public Port after(UserInfoInterface userInfo, ProcessData procData) {
		...
		try {
			...
			for (int i = 0; i < quadro6Values.size(); i++) {
				...
				if(quadro6Values.get(i).get("periodocarencia") != null &&
						(((String) quadro6Values.get(i).get("periodocarencia")).equals("E02") || ((String) quadro6Values.get(i).get("periodocarencia")).equals("E03") || ((String) quadro6Values.get(i).get("periodocarencia")).equals("E05"))) {
			...
			// db connection not needed anymore
			Logger.debug(login, this, "after", procData.getSignature() + "finished building XML");
			DatabaseInterface.closeResources(connection);
			...
			Logger.debug(login, this, "after", procData.getSignature() + "Finished writing temp file: " + tmpFile.getName());
			...
			Logger.debug(login, this, "after", procData.getSignature() + "Finished setting document contents");
			...
			Logger.debug(login, this, "after", procData.getSignature() + "Setting document in database. Docid = " + document.getDocId());
	...
	public static ArrayList<HashMap<String, Object>> fillAtributtes(XMLStreamWriter writer, Connection connection,
		...
		HashMap<String, Object> resultAux;
		...
		try {
			...
			while(rs.next()) {
				...
				for (int i = 1; i < (rsm.getColumnCount() + 1); i++) {
					...
					resultAux.put(column, value);

					if (column.endsWith("_id") || column.equals("id") || writer==null || value==null) {
						continue;
					} else if(StringUtils.equalsIgnoreCase(column,"type"))
						writer.writeAttribute("xsi","",column, rs.getString(i));
					else if (type == java.sql.Types.VARCHAR) {
						writer.writeAttribute(column, rs.getString(i));
					} else if (type == java.sql.Types.DATE) {
						SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
						writer.writeAttribute(column, sdf.format(rs.getDate(i)));
					} else if (type == java.sql.Types.TIMESTAMP) {
						SimpleDateFormat sdfDate = new SimpleDateFormat("yyyy-MM-dd");
						SimpleDateFormat sdfTime = new SimpleDateFormat("HH:mm:ss");
						String timeAux = sdfDate.format(rs.getTimestamp(i))+"T"+sdfTime.format(rs.getTimestamp(i));
						writer.writeAttribute(column, timeAux);
					} else if (type == java.sql.Types.DECIMAL) {
						DecimalFormat df = new DecimalFormat(
								"##################################################.############################");
						writer.writeAttribute(column, df.format(rs.getDouble(i)));
					} else if (type == java.sql.Types.BOOLEAN || type == java.sql.Types.TINYINT) {
						int valAux = rs.getBoolean(i) ? 1 : 0;
						writer.writeAttribute(column, "" + valAux);
					} else if (type == java.sql.Types.INTEGER) {
						writer.writeAttribute(column, String.format("%d", rs.getInt(i)));
					}
				}
	...
	public String formatField(HashMap<String,Object> quadro, String field, String type) {
		...
		if(type.equals("BigDecimal") || type.equals("BigDecimal5")) {//BigDecimal
			...
			if(type.equals("Date")) {//Date
				...
				LocalDate datetime = dateConverter(valor);
	...
	public LocalDate dateConverter(String valor) {
		LocalDate datetime;
		try {
			return datetime = LocalDate.parse(valor, DateTimeFormatter.ofPattern("dd/MM/yyyy"));
		}catch(Exception e) {
			try {
				return datetime = LocalDate.parse(valor, DateTimeFormatter.ofPattern("MM/dd/yyyy"));
			}catch(Exception ex) {
				try {
					return datetime = LocalDate.parse(valor, DateTimeFormatter.ofPattern("dd-MM-yyyy"));
				}catch(Exception ex1) {
		...
		return datetime = LocalDate.parse(valor, DateTimeFormatter.ofPattern("yyyy-MM-dd"));
	}
	...
	//Checks if string value is not null or zero
	public boolean checkNotZeroOrNull(String valor) {
		if(valor == null || valor.equals("") || valor.equalsIgnoreCase("null"))
			return false;

		Float valorFloat = new Float(valor);
