...
public class BlockVFSMarkAsTag extends Block {
  public Port portIn, portTrue, portFalse;

  private final String sATTR_DOCS = "Name";
  private final String sATTR_TOSIGN = "Mark";

  public BlockVFSMarkAsTag(int anFlowId, int id, int subflowblockid, String filename) {
  ...
  public Port after(UserInfoInterface userInfo, ProcessData procData) {
    ...
    try {
      String sTemplate = this.getAttribute(sATTR_DOCS);
      String sTemplateName = procData.transform(userInfo, sTemplate);
      String sSign = this.getAttribute(sATTR_TOSIGN);
      ...
      if (!StringUtils.equalsIgnoreCase("true", sSign) && !StringUtils.equalsIgnoreCase("false", sSign)) {
        ...
        Boolean marked = ctm.markAsTag(userInfo, sTemplateName, mark);

        if (!marked) {
          Logger.warning(login, this, "after", procData.getSignature() + "No template found to mark - " + sTemplate);
          outPort = portFalse;
        }
      }
    } catch (Exception e) {
