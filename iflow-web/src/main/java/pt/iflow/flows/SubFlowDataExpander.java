...
public class SubFlowDataExpander {
	...
	public Boolean containsSubFlow() {
		for (XmlBlockType block : getXmlFlow().getXmlBlock())
			if (block.getName().equals(SUB_FLOW_TYPE)) {
				return true;
			}
		return false;
	}

	private SubFlowDataSuportClass getSubFlowDataFromSubFlowBlock(UserInfoInterface userInfo, XmlBlockType subFlowBlock)
		...
		while (this.containsSubFlow()) {
			...
			if (subFlowElements != null && subFlowElements.size() > 0) {
				for (SubFlowDataSuportClass subFlowData : subFlowElements) {
					...
					Logger.debug(userInfo.getUtilizador(), this, "expandSubFlow",
							"added " + (blockMappings.size() - tempMappings) + " from Sub Flow: " + subFlow.getName());

					Logger.debug(userInfo.getUtilizador(), this, "expandSubFlow",
							"INI - create copy blocks in sub flow");
					buildCopyBlocksInSubFlow(subFlowData, userInfo);
					Logger.debug(userInfo.getUtilizador(), this, "expandSubFlow",
							"FIM - create copy blocks in sub flow");
	...
	private void validateSubFlow(XmlFlow xmlSubFlow, UserInfoInterface userInfo) throws Exception {
		...
		for (XmlBlockType block : xmlSubFlow.getXmlBlock()) {
			...
			// Logger.error(userInfo.getUtilizador(), this, "buildFlow", "exception caught:
			// " + "SubFlow" + xmlSubFlow.getName()
			// + " contains another inner SubFlow!");
			// throw new Exception("SubFlow"+ xmlSubFlow.getName() +" contains another inner
			// SubFlow!");
			// }
			if (block.getName().equals(BLOCK_END_TYPE))
				hasBlockEnd = Boolean.TRUE;
		}
		if (!hasBlockEnd) {
			Logger.error(userInfo.getUtilizador(), this, "buildFlow",
					"exception caught: " + "SubFlow" + xmlSubFlow.getName() + " must contain a BlockEnd!");
	...
	private void addSubCatalogVarsToMain(XmlFlow xmlSubFlow) {
		...
		for (XmlCatalogVarAttributeType xmlCatalogVarAttribute : xmlSubFlow.getXmlCatalogVars()
	...
	private void buildCopyBlocksInSubFlow(SubFlowDataSuportClass subFlowData, UserInfoInterface userInfo)
			throws Exception {
		...
		for (int in = 0; in < saInVars.length; in++) {
			for (int out = 0; out < saInVars.length; out++) {
				if (saInVars[in][FLOW].equals(saInVars[out][SUBFLOW]) && in != out) {
					Logger.error(userInfo.getUtilizador(), this, "buildFlow",
							"exception caught: In MainFlow - " + this.xmlFlow.getName() + ", SubFlow - "
									+ subFlowData.getSubFlowName()
									+ " variables with the same name must be mapped one to the other, varM - "
									+ saInVars[in][FLOW] + " varS - " + saInVars[in][SUBFLOW]);
	...
	/**
	 * Inserts subflow in main by adding subflow's expanded block array to main
	 * block array <br>
	...
	private void insertSubInMainFlow(SubFlowDataSuportClass subFlowData, UserInfoInterface userInfo) {
		...
		if (SubFlowDataSuportClass.SUBFLOW_BLOCK == subFlowData.getTypeOfSubFlowImplementation()) {
			...
			for (XmlBlockType block : xmlFlow.getXmlBlock()) {
				// if(block.getId()==subFlowBlock.getXmlPort(0).getConnectedBlockId()){ // é
				// bloco que liga ao subFluxo
		...
		if (SubFlowDataSuportClass.SUBFLOW_BLOCK == subFlowData.getTypeOfSubFlowImplementation()) {
			// 3.Add blocks to main flow ("Link's" the last block of sub flow to the block
			// conected to the subflowBlock from main flow)
			for (XmlBlockType block : xmlFlow.getXmlBlock()) {
	...
	private String[][] processMappingOfPopupVariables(List<XmlAttributeType> xmlAttributes) {
		...
		for (XmlAttributeType xmlAttribute : xmlAttributes) {
			...
			if (attributeName.startsWith("OBJ_") && attributeName.contains("_ROW_")) {
				...
				String variableLineNumber = attributeName.substring(variableLineNumberInitialIdx,
						attributeName.indexOf("_", variableLineNumberInitialIdx));
	...
	private XmlBlockType processSubFlowStartBlock(SubFlowDataSuportClass subFlowData, UserInfoInterface userInfo) {
		...
		subFlowBlockStart.setName(BLOCK_COPY_TYPE);
		...
		subFlowBlockStart.withXmlAttribute(new ArrayList<XmlAttributeType>());
	...
	private XmlBlockType processSubFlowEndBlock(SubFlowDataSuportClass subFlowData, UserInfoInterface userInfo) {
		...
		for (XmlBlockType block : rawSubFlow) {
			if (block.getName().equals(BLOCK_END_TYPE)) {
				...
				endId.add(block.getId().intValue());
			}
		}

		subFlowBlockEnd.setName(BLOCK_COPY_TYPE);
		...
		subFlowBlockEnd.withXmlAttribute(new ArrayList<XmlAttributeType>());

		return subFlowBlockEnd;
	}

	/**
	 * Method which runs the blocks of popup flow, including a property with the
	 * next block of the main flow
	 * 
	 * @param subFlowData
	 * @param userInfo
	 */
	private void insertPopupReturnBlockInInSubFlowBocksProperties(SubFlowDataSuportClass subFlowData,
			UserInfoInterface userInfo) {
		List<XmlBlockType> subFlowBlocks = subFlowData.getSubFlowXML().getXmlBlock();

		for (XmlBlockType block : subFlowBlocks) {
			block.setPopupReturnBlockId(subFlowData.getPrimaryBlock().getId());
	...
	private List<SubFlowDataSuportClass> getAllSubFlowsReferencesFromFlow(UserInfoInterface userInfo)
			throws JAXBException {
		...
		for (XmlBlockType block : getXmlFlow().getXmlBlock()) {
			SubFlowDataSuportClass subFlowData = null;
			if (SUB_FLOW_TYPE.equals(block.getName())) {
				Logger.debug(operationUser, this, methodName,
						"INI - Processing block of SubFlow." + block.getId() + ", " + getXmlFlow().getName());
				subFlowData = getSubFlowDataFromSubFlowBlock(userInfo, block);
				if (subFlowData == null) {
					subFlowData = new SubFlowDataSuportClass(SubFlowDataSuportClass.SUBFLOW_BLOCK, null, block, null,
							null, -1);
					Logger.error(operationUser, this, methodName,
							"ERROR - while processing sub flow data from SubFlowBlock");
				} else {
					Logger.debug(operationUser, this, methodName, "Info - Flow: " + getXmlFlow().getName()
							+ " will use subFlow " + subFlowData.getSubFlowName());
				}
				// missing ports..
			...
			if (BOCK_FORM_TYPE.equals(block.getName())) {
	...
	private List<SubFlowDataSuportClass> getSubFlowDataFromFormBlock(UserInfoInterface userInfo, XmlBlockType formBlock)
			throws JAXBException {
		String operationUser = userInfo.getUtilizador();
		String methodName = "getFlowSubFlows";
	...
	private List<List<XmlAttributeType>> getPopupFormFields(UserInfoInterface userInfo, XmlBlockType block) {
			...
			if (attributteName.startsWith("OBJ_")) {
				...
				if (attributteName.endsWith("_type")) {
					if ("pt.iflow.blocks.form.PopupFormField".equals(formAttribute.getValue())) {
						Logger.info(userInfo.getUtilizador(), this, "getPopupFormFields",
								"field [" + currentFieldId + "], is a popup");
		...
		if (formField != null) {
			formFieldsList.put(currentFieldId, formField);
			; // inclui o ultimo campo do formulario
		}
		...
		if (popupFormFieldsPositions.size() > 0) {
			...
			Logger.info(userInfo.getUtilizador(), this, "getPopupFormFields",
					"Processing fields [" + popupFormFieldsPositions.toString() + "], as popup fields");
	...
	private List<SubFlowMapping> assignNewBlockIds(SubFlowDataSuportClass subflow, Integer mainFlowMaxBlockId) {
		...
		for (XmlBlockType block : subflow.getSubFlowXML().getXmlBlock()) {
			mappings.add(new SubFlowMapping(xmlFlow.getName(), getSubFlowPrefix(block), block.getId().intValue(),
					block.getId().intValue() + mainFlowMaxBlockId));
			block.setId(BigInteger.valueOf(block.getId().intValue() + mainFlowMaxBlockId));
			for (XmlPortType xmlPort : block.getXmlPort())
				xmlPort.setConnectedBlockId(
						BigInteger.valueOf(xmlPort.getConnectedBlockId().intValue() + mainFlowMaxBlockId));
		}
		return mappings;
	}
	...
	public Integer findMaxblockId() {
		Integer result = 0;
		for (XmlBlockType block : xmlFlow.getXmlBlock())
			if (block.getId().intValue() > result)
				result = block.getId().intValue();
...
class SubFlowDataSuportClass {
	...
	public SubFlowDataSuportClass(int typeOfSubFlowImplementation, String subFlowName, XmlBlockType primaryBlock,
			XmlFlow subFlowXML, String popupVariable, int formFieldId) {
		super();
		this.typeOfSubFlowImplementation = typeOfSubFlowImplementation;
		this.subFlowName = subFlowName;
	...
	public void updatePopUpStartBlockId(XmlBlockType popupFlowStartupBlock, UserInfoInterface userInfo) {
		...
		StringBuffer attributeName = new StringBuffer("OBJ_").append(formFieldId).append("_")
				.append(PopupFormField.POPUP_FLOW_START_BLOCK_ID);
	...
	public String toString() {
		...
		if (primaryBlock != null) {
			...
			builder.append("tipo=").append(String.valueOf(primaryBlock.getName()));
			builder.append("}, ");
		}
