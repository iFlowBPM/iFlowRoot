...
public abstract class TransformUtility {
  ...
  /**
   * Constructor
   * @param reader FormXMLReader instance to use as event producer
   */
  ...
  /**
   * Create a new instance of TransformUtility using a FormXMLReader as SAX event producer
   * 
   * @param formReader FormXMLReader instance - SAX event producer
   ...
   * @param output transformation result destination
   * @param debugOutput XML output generated SAX events to this stream (XML not transformed). Use null to disable.
   * @throws TransformerException
   * @throws SAXException
   * @throws IOException
   */
  public abstract void transform(Writer output, Writer debugOutput) throws TransformerException,
      SAXException, IOException;

  /**
   * Retrieve a Templates instance from cache for this FormXMLReader instance.
   * This method first try to get a XSL configured in the form. If not found, try to fetch the default XSL for this flow.
   * Then try to fetch organization default XSL. If everything fails, return null.
   * 
   * @param reader The form, since it contains all information
  ...
  static Templates getTemplates(FormXMLReader reader) throws TransformerException {
    ...
    if (null == templates) {
      Logger.info(userInfo.getUtilizador(), "TransformUtility", "getTemplates", "Stylesheet not found. Trying flow stylesheet...");
      // some default XSL processing...
      String sDefaultXslName = "";
      FlowSetting defXSLSetting = BeanFactory.getFlowSettingsBean().getFlowSetting(reader.getProcData().getFlowId(),
          Const.sDEFAULT_STYLESHEET);
      if (null != defXSLSetting)
        sDefaultXslName = defXSLSetting.getValue();

      Logger.debug(userInfo.getUtilizador(), "TransformUtility", "getTemplates", "[" + procData.getFlowId() + "," + procData.getPid() + ","
    ...
    if (null == templates) {
      Logger.info(userInfo.getUtilizador(), "TransformUtility", "getTemplates", "Flow stylesheet not found. Fail back to system default...");
  ...
  /**
   * Transform XML using plain TRaX, ie, invoking transformer.transform(source, result).
   * This implemetation will use an auxilliary XMLReader and "identity" transformer to output debug information.
   */
  private static class TransformUtilityTRaX extends TransformUtility {
    ...
    @Override
    public void transform(Writer output, Writer debugOutput) throws TransformerException, SAXException,
        IOException {
  ...
  /**
   * Transform XML using TRaX and SAXTransformerFactory, allowing us to use ContentHandlers directly without transformers.
   * I believe it is more efficient than "regular" TRaX, but never tested this.
   */
