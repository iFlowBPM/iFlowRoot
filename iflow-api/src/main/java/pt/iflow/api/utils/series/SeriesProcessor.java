...
public abstract class SeriesProcessor {
	...
	public static final String DESCRIPTION_KEY_FORMAT = "series.{0}.description"; // 0:
																					// series
																					// processor
	public static final String STATE_DESCRIPTION_KEY_FORMAT = "series.state_descriptions.{0}"; // 0:
																								// series
																								// state
	...
	private void setEnabled(boolean enabled) throws SeriesException {
		...
		try {
			...
			try { if(null != db) db.close(); } catch (SQLException e2) {}
		    try { if(null != pstUpd) pstUpd.close(); } catch (SQLException e2) {}
		    try { if(null != pstGet) pstGet.close(); } catch (SQLException e2) {}
		    try { if(null != rs) rs.close(); } catch (SQLException e2) {}
